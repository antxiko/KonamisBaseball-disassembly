# Konami's Baseball (Konami, MSX1) - desensamblado
#
...
# cubre.
#
# La ROM no se distribuye. Hace falta en la raiz como baseball.rom, y
# `make comprueba` verifica el sha256.

ROM      = baseball.rom
SHA      = 06504efb72d1cd3351dae8eb1f7b073f69d599fa7018663937f85535658ad3e7
SRC      = src
WORK     = work
ORG      = 0x4000
TITULO   = KONAMI'S BASEBALL - Konami - MSX1 - cartucho RC-724 de 16 KB en la pagina 1
...
$(ROM):
	...
	@echo ""
	@echo " Es Konami's Baseball (Konami, RC-724) para MSX, 16384 bytes exactos."
...
$(WORK)/baseball.trace.json: $(ROM) $(SRC)/baseball.entries $(SRC)/baseball.nocode
	@mkdir -p $(WORK)
	python3 tools/z80trace.py $(ROM) $(ORG) $(SRC)/baseball.entries \
	        $(WORK)/baseball $(SRC)/baseball.nocode

trace: $(WORK)/baseball.trace.json

listado: $(WORK)/baseball.trace.json $(SRC)/baseball.notes
	python3 tools/mkasm.py $(ROM) $(ORG) $(WORK)/baseball.trace.json \
	        $(SRC)/baseball.notes work/msx.sym $(SRC)/baseball.asm "$(TITULO)"

# La prueba que decide si el desensamblado es fiable.
verify: $(SRC)/baseball.asm $(ROM)
	@sh tools/verify_build.sh $(SRC)/baseball.asm $(ROM) $(ORG)
...
densidad:
	@python3 tools/densidad.py $(SRC)/baseball.asm
...
imagenes: $(ROM)
	@mkdir -p work/gfx
	python3 tools/dibuja.py $(ROM) $(ORG) $(SRC)/baseball.notes work/gfx
...
clean:
	rm -rf $(WORK)/baseball.trace.json $(WORK)/baseball.blocks
